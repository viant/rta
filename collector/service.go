...
func (s *Service) Flush(batch *Batch) (string, error) {
	...
	if batch.Accumulator.Len() != 0 {
		...
		start1 := time.Now()
		var err error
		if totalStr, err = s.load(context.Background(), data, batch.ID); err != nil {
	...
	return fmt.Sprintf(" loadT: %v, fsloadT: %v, %s", elapsed1, elapsed2, totalStr), s.closeBatch(ctx, batch)
...
func (s *Service) load(ctx context.Context, data interface{}, batchID string) (string, error) {
	...
	return fmt.Sprintf("delay: %v, rLoadT: %v", delay, elapsed), err
}
...
func (s *Service) flushScheduledBatches(ctx context.Context) (flushed bool, err error) {
	...
	flushStart := time.Now()
	elapsedStr, err := s.Flush(masterBatch)
	...
	if err == nil {
		if s.config.Debug {
			fmt.Printf("succesfully flushed batch by collector [instance: %s, category: %s]: (rows cnt: %d, batches cnt: %d, mergeBatchT: %v, flushT: %v, flushScheduledBatches %v, %v) %v\n", s.instanceId, s.category, masterBatch.Accumulator.Len(), len(batchesToFlushNow), elapsed, flushElapsed, elapsed0, elapsedStr, masterBatch.ID)
		}
